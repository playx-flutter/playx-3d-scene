package io.sourcya.playx_3d_scene.core.camera
...
class CameraManger(
    ...
    fun updateCamera(cameraInfo: Camera?): Resource<String> {
        ...
        val res1 = updateExposure(exposure)
        Timber.d("camera Exposure: ${res1 is Resource.Success}")
        val projection = cameraInfo.projection
        val res2 = updateProjection(projection)
        Timber.d("camera updateProjection: ${res2.message}")

        val lensProjection = cameraInfo.lensProjection
        val res3 = updateLensProjection(lensProjection)
        Timber.d("camera updateLensProjection: ${res3 is Resource.Success}")

        val shift = cameraInfo.shift
        val res4 = updateCameraShift(shift)
        Timber.d("camera updateCameraShift: ${res4 is Resource.Success}")
        val scaling = cameraInfo.scaling
        val res5 = updateCameraScaling(scaling)
        Timber.d("camera updateCameraScaling: ${res5 is Resource.Success}")
    ...
    fun updateCameraManipulator(cameraInfo: Camera?) {
        if (cameraInfo == null) {
            return
        }
        ...
        if (cameraInfo.targetPosition != null) {
            val x = cameraInfo.targetPosition.getOrElse(0) { kDefaultObjectPosition.x }
            val y = cameraInfo.targetPosition.getOrElse(1) { kDefaultObjectPosition.y }
            val z = cameraInfo.targetPosition.getOrElse(2) { kDefaultObjectPosition.z }
        ...
        cameraInfo.upVector?.let {
            val x = it.getOrElse(0) { 0f }
            val y = it.getOrElse(1) { 1f }
            val z = it.getOrElse(2) { 0f }
        ...
        cameraInfo.orbitHomePosition?.let {
            val x = it.getOrElse(0) { 0f }
            val y = it.getOrElse(1) { 0f }
            val z = it.getOrElse(2) { 1f }
    ...
    fun setDefaultCamera() {
        ...
        val view = surfaceView ?: textureView
        Timber.d("Default camera: ${view == null}")
        view?.let {
            gestureDetector = GestureDetector(it, cameraManipulator)
        }
