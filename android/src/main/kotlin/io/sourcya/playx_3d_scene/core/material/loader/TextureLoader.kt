package io.sourcya.playx_3d_scene.core.material.loader
...
class TextureLoader(
        ...
        return withContext(Dispatchers.IO) {
            when (val bitmapResult =
                ...
                is Resource.Success -> {
                    ...
                    if (bitmap != null) {
                        Timber.d("loadTextureFromAsset : success $bitmap")
                        return@withContext loadTexture(bitmap, type)
                    } else {
                        Timber.d("loadTextureFromAsset : error : $")
    ...
    private fun loadTexture(bitmap: Bitmap, type: TextureType): Resource<Texture> {
        try {
            Timber.d("loadTextureFromAsset : loadTexture")
            ...
            Timber.d("loadTextureFromAsset : loadTexture success : $texture")

            return Resource.Success(texture)
        } catch (t: Throwable) {
            Timber.d("loadTextureFromAsset : loadTexture : ${t.message}")
